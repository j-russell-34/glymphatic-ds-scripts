...
if [ $skip -eq 0 ]; then
	...
	if [ -f "${json1}" ]; then
		...
		then
			...
			# For Philips scanners, try EstimatedTotalReadoutTime first, then TotalReadoutTime
			TotalReadoutTime1=$(cat "${json1}" | awk -F'"' '/"EstimatedTotalReadoutTime"/ {print $3}' | cut -d ':' -f2 | tr -d ',' | xargs)
			if [ -z "$TotalReadoutTime1" ]; then
				TotalReadoutTime1=$(cat "${json1}" | awk -F'"' '/"TotalReadoutTime"/ {print $3}' | cut -d ':' -f2 | tr -d ',' | xargs)
			fi
			if [ -z "$TotalReadoutTime1" ]; then
				TotalReadoutTime1=0.1 # Default fallback value
			fi
		else
			# For non-Philips scanners, try PhaseEncodingAxis first, then PhaseEncodingDirection (following GitHub issue #11 suggestion)
			PEdir1=$(cat "${json1}" | awk -F'"' '/"PhaseEncodingAxis"/ {print $4}')
			if [ -z "$PEdir1" ]; then
				# If PhaseEncodingAxis not found, try PhaseEncodingDirection
				PEdir1=$(cat "${json1}" | awk -F'"' '/"PhaseEncodingDirection"/ {print $4}')
			fi
			# Fix TotalReadoutTime parsing to remove closing brace
			TotalReadoutTime1=$(cat "${json1}" | awk -F'"' '/"TotalReadoutTime"/ {print $3}' | cut -d ':' -f2 | tr -d ',' | xargs)
			if [ -z "$TotalReadoutTime1" ]; then
				TotalReadoutTime1=0.1 # Default fallback value
			fi
			...
			##
   			if [ "$json2" ]; then
				...
				then
					...
					# For Philips scanners, try EstimatedTotalReadoutTime first, then TotalReadoutTime
					TotalReadoutTime2=$(cat "${json2}" | awk -F'"' '/"EstimatedTotalReadoutTime"/ {print $3}' | cut -d ':' -f2 | tr -d ',' | xargs)
					if [ -z "$TotalReadoutTime2" ]; then
						TotalReadoutTime2=$(cat "${json2}" | awk -F'"' '/"TotalReadoutTime"/ {print $3}' | cut -d ':' -f2 | tr -d ',' | xargs)
					fi
					if [ -z "$TotalReadoutTime2" ]; then
					...
					TotalReadoutTime2=$(cat "${json2}" | awk -F'"' '/"TotalReadoutTime"/ {print $3}' | cut -d ':' -f2 | tr -d ',' | xargs)
					if [ -z "$TotalReadoutTime2" ]; then
						TotalReadoutTime2=0.1 # Default fallback value
					fi
				fi
			...
			then
				...
				#EDDY
					...
					if [ $eddy == "1" ] && [ -f ${FSLDIR}/bin/eddy_cpu ]; then 
						echo "Found eddy_cpu! Running ${FSLDIR}/bin/eddy_cpu with --data_is_shelled flag"
						eddy_cpu --imain="$dwi1_processed" --mask=b0_brain_mask --acqp=acqparams.txt --index=index.txt --bvecs=bvec1 --bvals=bval1 --topup=my_topup_results --data_is_shelled --out=eddy_corrected_data
					elif [ $eddy == "2" ] && [ -f ${FSLDIR}/bin/eddy ]; then 
						echo "Running ${FSLDIR}/bin/eddy with --data_is_shelled flag";
						eddy --imain="$dwi1_processed" --mask=b0_brain_mask --acqp=acqparams.txt --index=index.txt --bvecs=bvec1 --bvals=bval1 --topup=my_topup_results --data_is_shelled --out=eddy_corrected_data
				...
				#EDDY
					...
					if [ $eddy == "1" ] && [ -f ${FSLDIR}/bin/eddy_cpu ]; then 
							echo "Found eddy_cpu! Running ${FSLDIR}/bin/eddy_cpu with --data_is_shelled flag"
							eddy_cpu --imain="$dwi1_processed" --mask=b0_brain_mask --acqp=acqparams.txt --index=index.txt --bvecs=bvec1 --bvals=bval1 --data_is_shelled --out=eddy_corrected_data
					elif [ $eddy == "2" ] && [ -f ${FSLDIR}/bin/eddy ]; then 
							echo "Running ${FSLDIR}/bin/eddy with --data_is_shelled flag";
							eddy --imain="$dwi1_processed" --mask=b0_brain_mask --acqp=acqparams.txt --index=index.txt --bvecs=bvec1 --bvals=bval1 --data_is_shelled --out=eddy_corrected_data
					elif [ $eddy == "3" ] && [ -f ${FSLDIR}/bin/eddy_correct ]; then 
							echo "Running ${FSLDIR}/bin/eddy_correct with default options";
							eddy_correct "$dwi1_processed" eddy_corrected_data 0 trilinear
					else echo "Eddy with user-specified eddy program ${FSLDIR}/bin/$eddy"
