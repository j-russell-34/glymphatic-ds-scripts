...
#SBATCH --job-name=dti_manual_rois
#SBATCH --array=1-4
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
#SBATCH --output=logs/manual_rois_%A_%a.log
...
if [ ! -f "$SUBJECT_LIST_FILE" ]; then
    echo "Error: Subject list not found. Please run subject_list_alps_qc.sh first"
    exit 1
fi
...
apptainer exec \
    ...
    "$CONTAINER" \
    bash /data/scripts/manual_rois.sh ${SUBJECT_ID}
