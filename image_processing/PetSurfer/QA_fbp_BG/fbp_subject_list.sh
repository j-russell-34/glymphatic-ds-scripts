...
# Generate the subject list, including only those with 'fbp' subfolders
echo "Generating subject list from: $SUBJECTS_BASE_DIR"

# Find all directories that have 'fbp' or 'fbb' subdirectories
find "$SUBJECTS_BASE_DIR" -mindepth 2 -maxdepth 2 -type d \( -name "fbp" -o -name "fbb" \) ! -path "*/fsaverage/*" | sed 's|/fbp$||;s|/fbb$||' | sed "s|$SUBJECTS_BASE_DIR/||" > "$SUBJECT_LIST_FILE"
