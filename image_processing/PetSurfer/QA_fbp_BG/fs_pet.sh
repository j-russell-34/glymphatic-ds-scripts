...
QA_DIR="$OUTPUT_DIR/Fbp_QA_BG_Reports"
mkdir -p "$QA_DIR"
...
generate_pet_qa_images() {
    ...
    if [ ! -f "$SUBJECTS_PET_DIR/$subject/gtmpvc_wm_rr.output/rbv.nii.gz" ]; then
        echo "Error: rbv.nii.gz not found for $subject/$pet_dir"
        return 1
    fi
    
    if [ ! -f "$SUBJECTS_PET_DIR/$subject/gtmpvc_wm_rr.output/input.rescaled.nii.gz" ]; then
    ...
    xvfb-run -a --server-args "-screen 0 1920x1080x24" freeview -v \
        ...
        "$SUBJECTS_PET_DIR/$subject/gtmpvc_wm_rr.output/rbv.nii.gz":colormap=heat:opacity=0.5 \
        "$SUBJECTS_MRI_DIR/$subject/mri/gtmseg+ewm.mgz":colormap=lut:lut=$SCRIPT_DIR/ewm_rr.ctab:opacity=0.7 \
    ...
    xvfb-run -a --server-args "-screen 0 1920x1080x24" freeview -v \
        ...
        "$SUBJECTS_PET_DIR/$subject/gtmpvc_wm_rr.output/rbv.long.nii.gz":colormap=heat:opacity=0.5 \
        "$SUBJECTS_PET_DIR/$subject/basal_ganglia.nii.gz":colormap=lut:lut=$SCRIPT_DIR/basal_ganglia.ctab:opacity=0.7 \
        "$SUBJECTS_MRI_DIR/$subject/mri/gtmseg+ewm_long.mgz":colormap=lut:lut=$SCRIPT_DIR/ewm_rr.ctab:opacity=0.7 \
        --layout 1 --viewport sagittal \
    ...
    # Create a PDF page for this subject 
    montage \
