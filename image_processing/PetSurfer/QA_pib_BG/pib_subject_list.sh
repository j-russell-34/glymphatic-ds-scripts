...
SUBJECT_LIST_FILE="/project2/jasonkru_1564/studies/${STUDY}/pib_subject_list.txt"
...
# Generate the subject list, including only those with 'ftp' subfolders
echo "Generating subject list from: $SUBJECTS_BASE_DIR"

# Find all directories that have 'pib' subdirectories
find "$SUBJECTS_BASE_DIR" -mindepth 2 -maxdepth 2 -type d \( -name "pib" \) ! -path "*/fsaverage/*" | sed 's|/pib$||' | sed "s|$SUBJECTS_BASE_DIR/||" > "$SUBJECT_LIST_FILE"

# Remove any empty lines or hidden files
sed -i '/^$/d;/^\./d' "$SUBJECT_LIST_FILE"

# Remove duplicates from the subject list
sort -u "$SUBJECT_LIST_FILE" -o "$SUBJECT_LIST_FILE"
