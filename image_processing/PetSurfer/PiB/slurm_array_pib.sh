...
#SBATCH --output=logs/pib_%A_%a.log
...
if [ ! -f "$SUBJECT_LIST_FILE" ]; then
    echo "Error: Subject list not found. Please run pib_subject_list.sh first"
    exit 1
fi
...
apptainer exec \
    ...
    -B /home1/jasonkru/atri_code/processors/PetSurfer/PiB:/data/scripts \
...
apptainer exec \
    ...
    "$CONTAINER2" \
    bash /data/scripts/run_pib.sh ${SUBJECT_ID} ${RADIOTRACER} ${PSF_CSV}