...
fw_cp_data <- read_csv(paste0(outputs, "/concatenated_choroid_plexus_FW_eroded.csv"))
...
#remove rows with 0 in wm_FW or choroid_plexus_FW
fw_data <- fw_data %>%
  filter(wm_FW > 0, choroid_plexus_FW > 0)
...
write_csv(fw_summary, paste0(harmonized_dir, "/fw_eroded_summary_harmonized.csv"))
