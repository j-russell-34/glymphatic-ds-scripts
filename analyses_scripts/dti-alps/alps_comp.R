...
ggsave(glue("{out_dir}/alps/alps_baseline_comparison_plot.svg"), width = 10, height = 8)
...
#make new dataframe filtering on subject_label in control_match_df
...
#drop families with only one entry (unpaired))
...
ggsave(glue("{out_dir}/alps/alps_paired_t_test_plot.svg"), width = 10, height = 8)
...
ggsave(glue("{out_dir}/alps/alps_longitudinal_comparison_plot.svg"), width = 10, height = 8)
...
ggplot() +
  ...
  theme_minimal(base_size = 36) +
  geom_line(data = alps_df_cog, aes(x = alps, y = trs, group = subject), alpha = 0.3, inherit.aes = FALSE) +
  theme(
    axis.title.y = element_markdown(),
    panel.grid = element_blank(),
    axis.line = element_line(color = "black"),
...
ggsave(glue("{out_dir}/alps/alps_mcrt_plot.svg"), width = 10, height = 8)
...
ggplot() +
  ...
  theme_minimal(base_size = 36) +
  geom_line(data = alps_df_cog, aes(x = alps, y = dsmse_to2, group = subject), alpha = 0.3, inherit.aes = FALSE) +
  theme(
    axis.title.y = element_markdown(),
    panel.grid = element_blank(),
    axis.line = element_line(color = "black"),
...
ggsave(glue("{out_dir}/alps/alps_dsmse_plot.svg"), width = 10, height = 8)
...
#select just participants with down syndrome from alps_df
alps_df_ds <- alps_df %>%
  filter(group == 1)
#test effect of sex on alps in ds_baseline
t_test_sex_alps <- t.test(alps ~ de_gender, data = alps_df_ds)
