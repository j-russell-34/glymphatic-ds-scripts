...
harmonized_dir <- "/Users/jasonkru/Documents/outputs/ABCDS/basal_ganglia_pib"
...
#import imaging data
pib_df = read_csv(paste0(outputs, "concat_gtm_stats_pib.csv"))
bg_suvr_df = read_csv(paste0(outputs, "pib_basal_ganglia.csv"))
...
#change subject_id to fsid in pib_df
pib_df <- pib_df %>%
  rename(fsid = subject_id)  

#join bg_suvr_df to pib_df on fsid
pib_df <- inner_join(pib_df, bg_suvr_df, by = "fsid")

#longCombat formulas choke on feature names that contain dashes, so make them syntactic
orig_feature_names <- setdiff(colnames(pib_df), "fsid")
clean_feature_names <- make.names(orig_feature_names, unique = TRUE)
colnames(pib_df)[match(orig_feature_names, colnames(pib_df))] <- clean_feature_names
...
#merge pib and dems_df on fsid
pib_df <- left_join(pib_df, dems_df, by="fsid")

#drop rows with NA in pib_df
pib_df <- pib_df %>%
  drop_na()

#inner join site_scanner_filtered and pib df on fsid
pib_df <- inner_join(site_scanner_filtered, pib_df, by = "fsid")
...
#inner join mri_latency_df and pib_df on fsid
pib_df <- inner_join(pib_df, mri_latency_df, by = "fsid")

#make table and count instances of consortium_site_id
table(pib_df$consortium_site_id)
...
combat_pib <-longCombat(idvar='subject_label',
                         timevar='mri_latency',
...
pib_harmonized <- combat_pib$data_combat
...
pib_harmonized$subject_label <- as.character(pib_harmonized$subject_label)
...
#inner join consensus_brief and pib_harmonized on event_sequence and subject_label
pib_harmonized_to_csv <- inner_join(consensus_brief, pib_harmonized, by = c("mri_latency", "subject_label"))

#save pib_harmonized
write_csv(pib_harmonized_to_csv, paste0(harmonized_dir, "/pib_bg_harmonized.csv"))