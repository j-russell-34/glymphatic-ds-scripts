...
#lmem with basal ganglia fbp as outcome and alps_harmonized, age, sex as fixed effects and site_id as random effect
lmem_bg_bgfbp <- lme(alps_harmonized ~ Basal_Ganglia_SUVR.combat + age + sex + site_id, random =  ~1|subject, data = merged_df, method = "REML")
summary(lmem_bg_bgfbp)
intervals(lmem_bg_bgfbp, which = "fixed")
...
lmem_cp_bgfbp <- lme(choroid_plexus_FW.combat ~ Basal_Ganglia_SUVR.combat + age + sex + site_id, random = ~1|subject, data = merged_df, method = "REML")
summary(lmem_cp_bgfbp)
intervals(lmem_cp_bgfbp, which = "fixed")

#using effects package plot linear relationship between alps_harmonized and Basal_Ganglia_SUVR.combat

alps_effect <- Effect(c("Basal_Ganglia_SUVR.combat"), lmem_bg_bgfbp)
...
ggsave(glue("{bg_dir}/bg_cpfwf_plot.svg"), plot, width = 10, height = 8)
